...
### sudo ./debcin.sh komutu ile çalıştırınız
...
chroot kaynak apt-get install firmware-linux firmware-linux-free firmware-linux-nonfree firmware-misc-nonfree firmware-amd-graphics firmware-realtek -y
...
chroot kaynak apt-get install gnome-terminal cinnamon wget synaptic -y

### İsteğe bağlı paketleri kuralım
#chroot kaynak apt-get install gvfs-backends inxi wget mintstick gnome-calculator file-roller -y
...
chroot kaynak apt remove xterm -y
...
grub-mkrescue isowork -o debian-cinnamon-live-$(date +%x).iso
