...
SUPPORTED_FORMATS = ('.mp3', '.wav', '.flac', '.m4a', '.mpga', '.aac', '.ogg', '.opus', '.wma', '.m4b', '.aiff', '.mid', '.amr', '.ape', '.wv', '.webm')
ICON_NAME = "turkamp.png" 

# --- Mevcut DragDropList ve ScrollingLabel sınıfları aynı kalıyor ---
class DragDropList(QListWidget):
    ...
    def show_context_menu(self, position):
        ...
        remove_action = QAction("Parçayı Sil", self); remove_action.triggered.connect(lambda: self.deleteRequested.emit())
        clear_action = QAction("Tümünü Sil", self); clear_action.triggered.connect(lambda: self.clearRequested.emit())
        if self.itemAt(position): menu.addAction(remove_action)
        menu.addAction(clear_action); menu.exec(self.mapToGlobal(position))

class ScrollingLabel(QWidget):
    def __init__(self, text="", parent=None):
        super().__init__(parent)
        self.full_text = ""; self.text_width = 0; self.offset = 0; self.setFixedHeight(30)
...
class ModernSpectrum(QWidget):
    ...
    def __init__(self, player):
        super().__init__(); self.player = player; self.color = QColor("#00e676"); self.bars = 35; self.mode = 0; self.heights = [0.0] * self.bars; self.target_heights = [0.0] * self.bars; self.timer = QTimer(); self.timer.timeout.connect(self.animate); self.timer.start(30)
    def mousePressEvent(self, event): self.mode = (self.mode + 1) % 10; self.modeChanged.emit(self.mode); self.update()
    def animate(self):
        ...
        self.update()
    def paintEvent(self, event):
        painter = QPainter(self); painter.setRenderHint(QPainter.RenderHint.Antialiasing); painter.fillRect(self.rect(), QColor("#000000"))
        w = self.width() / self.bars; mid_y = self.height() / 2
        for i in range(self.bars):
            h = self.heights[i]; grad = QLinearGradient(QPointF(0, float(self.height())), QPointF(0, float(self.height() - h)))
            grad.setColorAt(0, self.color); grad.setColorAt(1, QColor(255, 255, 255, 180)); painter.setBrush(grad); painter.setPen(Qt.PenStyle.NoPen)
            # Mod çizimleri basitleştirildi
            r = QRectF(i * w + 2, self.height() - h if self.mode != 1 else mid_y - h/2, w - 4, h)
            if self.mode < 3: painter.drawRoundedRect(r, 2, 2)
            else: painter.drawRect(r)

class TurkaPlayer(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle("Turka Music Player"); QGuiApplication.setDesktopFileName("turkamp")
        ...
        self.current_theme_idx = 0; self.collapsed_width = 440; self.expanded_width = 850; self.player_height = 560
        self.init_ui(); self.setup_logic(); self.load_settings(); self.apply_theme_styles(); self.toggle_list(force=self.is_list_visible); self.center_window()

    def center_window(self):
        qr = self.frameGeometry(); cp = QGuiApplication.primaryScreen().availableGeometry().center(); qr.moveCenter(cp); self.move(qr.topLeft())

    def init_ui(self):
        main_widget = QWidget(); self.setCentralWidget(main_widget); self.layout_horizontal = QHBoxLayout(main_widget); self.layout_horizontal.setContentsMargins(15, 15, 15, 15); self.layout_horizontal.setSpacing(15)
        ...
        # LCD Panel - Bitrate Bilgisi Eklendi
        self.lcd_container = QFrame(); self.lcd_container.setObjectName("LCDContainer"); self.lcd_container.setFixedHeight(200) 
        ...
        # Yeni Bilgi Satırı (Bitrate, Format vs.)
        self.info_lbl = QLabel("Bekliyor..."); self.info_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.info_lbl.setStyleSheet("color: #888; font-family: 'Monospace'; font-size: 11px; font-weight: bold;")
        lcd_lyt.addWidget(self.info_lbl)
        ...
        v_lyt = QHBoxLayout(self.volume_container); v_lyt.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.btn_vol_down = self.create_circle_btn("-", 38); self.knob = ProVolumeKnob(); self.btn_vol_up = self.create_circle_btn("+", 38)
        v_lyt.addWidget(self.btn_vol_down); v_lyt.addWidget(self.knob); v_lyt.addWidget(self.btn_vol_up); self.layout_left.addWidget(self.volume_container)
        
        self.nav_container = QFrame(); self.nav_container.setObjectName("NavPanel"); self.nav_container.setFixedHeight(85)
        n_lyt = QHBoxLayout(self.nav_container); n_lyt.setSpacing(12); n_lyt.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        for b in [self.btn_back5, self.btn_prev, self.btn_play, self.btn_next, self.btn_fwd5]: n_lyt.addWidget(b)
    ...
    # Bitrate ve Ses Bilgisi Alma Fonksiyonu
    def get_audio_info(self, file_path):
        if not MutagenFile: return "Mutagen kütüphanesi eksik"
        try:
            audio = MutagenFile(file_path)
            if audio is None: return "Bilinmeyen Format"
            
            ext = os.path.splitext(file_path)[1].upper()[1:]
            info = audio.info
            bitrate = getattr(info, 'bitrate', 0) // 1000
            samplerate = getattr(info, 'sample_rate', 0) / 1000
            channels = "Stereo" if getattr(info, 'channels', 2) == 2 else "Mono"
            
            # Kayıpsız formatlar (FLAC, WAV) için bitrate bazen farklı hesaplanır
            if bitrate == 0 and hasattr(info, 'bits_per_sample'):
                # Bitrate = SampleRate * BitsPerSample * Channels
                bitrate = int((info.sample_rate * info.bits_per_sample * info.channels) // 1000)

            return f"{ext} | {bitrate} kbps | {samplerate:.1f} kHz | {channels}"
        except:
    ...
    def play_file(self, item):
        ...
        if path and os.path.exists(path): 
            # Bilgiyi güncelle
            self.info_lbl.setText(self.get_audio_info(path))
    ...
    def toggle_list(self, force=None):
        ...
        self.setFixedSize(self.expanded_width if self.is_list_visible else self.collapsed_width, self.player_height); self.apply_theme_styles()
    ...
    def apply_theme_styles(self):
        ...
        if self.is_dark_mode:
            bg_style = "QMainWindow { background-color: #121212; }"; panel_bg = "#1e1e1e"; btn_grad = "stop:0 #333, stop:1 #1a1a1a"; shadow_light = "#2a2a2a"; shadow_dark = "#000000"; text_color = "#ffffff"
        else:
            bg_style = "QMainWindow { background-color: #e0e5ec; }"; panel_bg = "#e0e5ec"; btn_grad = "stop:0 #ffffff, stop:1 #d1d9e6"; shadow_light = "#ffffff"; shadow_dark = "#b8b9be"; text_color = "#333333"
        self.setStyleSheet(bg_style)
        self.centralWidget().setStyleSheet(f"QFrame#VolumePanel, QFrame#NavPanel {{ background-color: {panel_bg}; border-radius: 20px; border: 1px solid {shadow_light if self.is_dark_mode else '#ffffff'}; border-bottom: 5px solid {shadow_dark}; border-right: 2px solid {shadow_dark}; }} QFrame#LCDContainer {{ background-color: #000; border-radius: 15px; border: 4px solid {color}; }}")
        btn_base = f"QPushButton {{ background: qlineargradient(x1:0, y1:0, x2:0, y2:1, {btn_grad}); border: 1px solid {shadow_light}; color: {text_color}; font-weight: bold; border-bottom: 4px solid {shadow_dark}; outline: none; }} QPushButton:hover {{ border-color: {color}; }} QPushButton:pressed {{ background: {shadow_dark}; }}"
        for b in [self.btn_vol_down, self.btn_vol_up, self.btn_back5, self.btn_prev, self.btn_next, self.btn_fwd5]: b.setStyleSheet(btn_base.replace("{", "{ border-radius: 19px;"))
        self.btn_play.setStyleSheet(btn_base.replace("{", "{ border-radius: 32px;"))
        self.btn_play.setText("❚❚" if self.player.playbackState() == QMediaPlayer.PlaybackState.PlayingState else "▶")
        for b in [self.btn_add, self.btn_theme, self.btn_mode, self.btn_list_toggle, self.btn_shuffle, self.btn_repeat]: b.setStyleSheet(btn_base.replace("{", "{ border-radius: 8px;"))
        ...
        self.list.setStyleSheet(f"QListWidget {{ background: {panel_bg}; color: {text_color}; border-radius: 15px; border: 2px solid {shadow_dark}; selection-background-color: {color}; }}")
        self.progress_bar.setStyleSheet(f"QSlider::groove:horizontal {{ background: #111; height: 6px; border-radius: 3px; }} QSlider::handle:horizontal {{ background: {color}; width: 16px; margin: -5px 0; border-radius: 8px; }}")
        self.time_lbl.setStyleSheet(f"color: {color}; font-family: 'Monospace'; font-weight: bold;"); self.info_lbl.setStyleSheet(f"color: {color}; opacity: 0.7; font-family: 'Monospace'; font-size: 11px;")
    ...
    def setup_logic(self):
        ...
        self.btn_list_toggle.clicked.connect(lambda: self.toggle_list()); self.btn_shuffle.clicked.connect(self.toggle_shuffle); self.btn_repeat.clicked.connect(self.toggle_repeat)
        ...
        self.list.itemDoubleClicked.connect(self.play_file); self.btn_play.clicked.connect(self.toggle_play); self.btn_next.clicked.connect(self.next_track); self.btn_prev.clicked.connect(self.prev_track)
        self.btn_back5.clicked.connect(lambda: self.player.setPosition(max(0, self.player.position() - 5000))); self.btn_fwd5.clicked.connect(lambda: self.player.setPosition(min(self.player.duration(), self.player.position() + 5000)))
        ...
        self.progress_bar.sliderMoved.connect(self.player.setPosition); self.player.playbackStateChanged.connect(self.apply_theme_styles); self.player.mediaStatusChanged.connect(self.handle_media_end)
        self.list.fileDropped.connect(self.handle_dropped_files); self.list.deleteRequested.connect(self.remove_selected_item); self.list.clearRequested.connect(self.clear_playlist); self.search_bar.textChanged.connect(self.filter_playlist)

    # --- Yardımcı fonksiyonlar (load/save/manual_add vb.) önceki kodla aynı ---
    ...
    def manual_add(self):
      files, _ = QFileDialog.getOpenFileNames(self, "Müzik Seç", "", "Ses Dosyaları (*.mp3 *.wav *.flac *.m4a *.ogg *.opus *.wma)")
      if files: 
    ...
    def handle_dropped_files(self, paths):
        ...
        self.save_settings()
    def change_theme(self): self.current_theme_idx = (self.current_theme_idx + 1) % len(self.themes); self.apply_theme_styles(); self.save_settings()
    def toggle_mode(self): self.is_dark_mode = not self.is_dark_mode; self.btn_mode.setText("☾" if self.is_dark_mode else "☼"); self.apply_theme_styles(); self.save_settings()
    ...
    def save_settings(self):
        ...
        data = {"theme_index": self.current_theme_idx, "volume": self.knob.value, "playlist": playlist, "is_dark": self.is_dark_mode, "is_shuffled": self.is_shuffled, "is_repeated": self.is_repeated, "is_list_visible": self.is_list_visible, "current_index": self.list.currentRow(), "spectrum_mode": self.vumeter.mode}
        try:
        ...
        if os.path.exists(CONFIG_FILE):
            try:
                with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                    data = json.load(f); self.current_theme_idx = data.get("theme_index", 0); self.is_dark_mode = data.get("is_dark", True); self.is_shuffled = data.get("is_shuffled", False); self.is_repeated = data.get("is_repeated", False); self.is_list_visible = data.get("is_list_visible", False)
                    v = data.get("volume", 75); self.knob.setValue(v); self.audio.setVolume(v/100); self.vumeter.mode = data.get("spectrum_mode", 0)
